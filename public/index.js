...
class Tasklist {
  ...
  onAddTaskButtonClick = async () => {
    const newTaskName = prompt('Введите ФИО клиента:');
  ...
  onEditTask = async (taskID) => {
    ...
    const newTaskName = prompt('Введите новое ФИО клиента', oldTaskName);
  ...
  onDeleteTaskButtonClick = async (taskID) => {
    ...
    if (!confirm(`Клиент '${taskName}' будет удален. Продолжить?`)) return;
